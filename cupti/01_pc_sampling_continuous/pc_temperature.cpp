...
typedef struct ContextInfo_st
{
    ...
    char* deviceTemperature;
} ContextInfo;

// typedef struct PCTemp_st {
//     std::vector<CUpti_PCSamplingConfigurationInfo> pcSamplingConfigurationInfo;
    ...
    ContextInfo *pContextInfo)
{
    ...
    if (result != NVML_SUCCESS)
    {
        std::cerr << "Error: " << "Failed to get handle for device" << " - " << nvmlErrorString(result) << std::endl;
        exit(1);
    }
    ...
    if (result != NVML_SUCCESS)
    {
        std::cerr << "Error: " << "Failed to get temperature" << " - " << nvmlErrorString(result) << std::endl;
        exit(1);
    }

    char* ptr = (char*)&temperature;

    pContextInfo->deviceTemperature = ptr;
    ...
    if (result != NVML_SUCCESS)
    {
        std::cerr << "Error: " << "Failed to shutdown NVML" << " - " << nvmlErrorString(result) << std::endl;
        exit(1);
    }
...
StorePcSampDataInFile()
{
    ...
    // CONSIDERA CHE QUI PUOI AGGIUNGERE L'INFORMAZIONE SULLA TEMPERATURA.
    functions.insert(pContextInfo->deviceTemperature);
    //
